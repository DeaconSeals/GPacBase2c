...
class shortestPathPillAgent():
	...
	def select_action(self, game, player='m'):
		if len(self.pre_planned_actions) == 0:
			self.pre_planned_actions = path_to_pill_a_star(player, game)
...
class shortestPathFruitAgent(shortestPathPillAgent):
	def select_action(self, game, player='m'):
		if game.fruit_location is not None and len(self.pre_planned_actions) == 0:
			self.pre_planned_actions = path_to_fruit_a_star(player, game)
...
class ChasingGhostAgent:
	...
	def select_action(self, game: gpac.GPacGame, player: str) -> str:
		...
		if self.path_target is None or len(self.pre_planned_actions) == 0 or self.path_staleness_check(game, player):
			if random.random() < self.random_wander_chance:
				...
				self.path_target = game.players[nearest_pac_man(game, player)]

		return self.pre_planned_actions.popleft()

	def path_staleness_check(self, game: gpac.GPacGame, player: str) -> bool:
		...
		target_inaccuracy = manhattan_distance(self.path_target, game.players[nearest_pac_man(game, player)])
...
class AvoidingPacmanAgent(shortestPathFruitAgent):
	...
	def select_action(self, game: gpac.GPacGame, player: str = 'm') -> str:
		...
		if nearest_manhattan_distance(game.players[player], ghosts) <= self.avoidance_radius:
			self.pre_planned_actions = deque()
		if path_age > self.maximum_path_age:
			self.pre_planned_actions = deque()
		if game.fruit_location is not None and len(self.pre_planned_actions) == 0:
			fruit_path = path_to_fruit_a_star(player, game, ghost_proximity_cost_function)
			pill_path = path_to_pill_a_star(player, game, ghost_proximity_cost_function)
			...
			self.pre_planned_actions = path_to_pill_a_star(player, game, ghost_proximity_cost_function)
			self.path_timestamp = game.time
		return self.pre_planned_actions.popleft()
...
def path_to_pacman(player: str, game: gpac.GPacGame) -> deque:
	"""Search for the shortest path to the nearest pac-man using A*."""
	return path_to_point(game.players[player], game.players[nearest_pac_man(game, player)], game)


def nearest_pac_man(game: gpac.GPacGame, player: str):
	"""Returns the nearest living pac-man to the player using Manhattan distance."""
	pac_mans = [other_player for other_player in game.players if 'm' in other_player if other_player not in game.graveyard]
	if len(pac_mans) == 1:
		return pac_mans[0]
	return min(pac_mans, key=lambda pac_man: manhattan_distance(game.players[player], game.players[pac_man]))